...
namespace MyJetWallet.Connector.Bitgo.WsEngine
{
    ...
    public class WebsocketEngine : IDisposable
    {
        ...
        public WebsocketEngine(string name, string url, string authToken, int pingIntervalMSec, int silenceDisconnectIntervalMSec,
            ILogger logger)
        {
            _name = name;
            _url = url;
        ...
        public async Task Run()
        {
            while (!_token.IsCancellationRequested)
            {
                try
                {
                    ...
                    _clientWebSocket.Options.SetRequestHeader("Authorization", $"Bearer {_authToken}");
                    _clientWebSocket.Options.KeepAliveInterval = TimeSpan.FromMilliseconds(_pingIntervalMSec);
        ...
        private async Task CheckActivity(ClientWebSocket webSocket)
        {
            ...
            {
                _logger.LogError(ex, "Web socket {name} receive Exception on check activity", _name);
            }
        }
